...
TSP::TSP(std::vector<city> cities,std::vector<path> paths,Random rnd):_cities(std::move(cities)),_paths(std::move(paths)),_rnd(rnd){}
...
int TSP::Selection(){
    ...
    return _paths.size()*pow(_rnd.Rannyu(),6);
}
...
void TSP::NewGeneration(){
    ...
    for(unsigned int k{start};k<_paths.size();k+=2){
        ...
        //Crossover
        if(_rnd.Rannyu()<0.8){//80% of the probability to perform crossover
...
//Simulated Annealing
void TSP::SimulatedAnnealing(const double& beta, path& way) {
    // Create a copy of the original path
    path appo_way{way}; // Copy constructor

    double p1{},p2{},p3{},p4{};
    //Probability of pair permutation
    double calculation{1/(sqrt(beta)+1)};// at beta = 1, calc = 0.5

    if(beta <1){
        //at low beta (high temp) p1 is close to 1
        p1 = 0.2;
        p2 = 1-calculation; // p2>0.5 
        p3 = 1-calculation; // p3>0.5
        p4 = 0.2;
    }else{
        //at high beta (low temp) p1 is close to 0
        p1 = calculation; // p1>0.5
        p2 = 0.05;
        p3 = 0.05;
        p4 = calculation; // p4>0.5
    } 
    // Mutate the copied path
    Mutate(appo_way,p1,p2,p3,p4);

    double cost{way.LengthL2(_cities)}; // Calculate the cost of the original path
    double appo_cost{appo_way.LengthL2(_cities)}; // Calculate the cost of the mutated path
    double delta {appo_cost - cost}; // Calculate the change in cost

    // Apply the Metropolis algorithm
    if(delta < 0){
        //It's better, so we accept it
        way = std::move(appo_way); // Move assignment
    } else {
        //try to accept anyway
        if (_rnd.Rannyu() < exp(-beta * delta)) {
            way = std::move(appo_way); // Move assignment
        }
    }
...
//Mutations
path& TSP::Mutate(path& way,double p1,double p2,double p3,double p4){
    if(_rnd.Rannyu()< p1){
        //swap two random cities
        way.swap(_rnd.Rannyu(0,_cities.size()-1),_rnd.Rannyu(0,_cities.size()-1));
    }
    if(_rnd.Rannyu()<p2){
        // m>=1 to avoid the shift of a single city
        //shift starting from the i-th city m contiguous cities of n positions 
        way.shift(_rnd.Rannyu(0,_cities.size()-1),_rnd.Rannyu(1,_cities.size()-1),_rnd.Rannyu(0,_cities.size()-1));
    }
    if(_rnd.Rannyu()<p3){
        // m>=1 to avoid the permutation of a single city(m=1 can act as a swap)
        //permutation among m contiguous cities starting from the start-th city and separated by sep cities
        way.Permutation(_rnd.Rannyu(1,_cities.size()-1),_rnd.Rannyu(0,_cities.size()-1),_rnd.Rannyu(0,_cities.size()-1));
    }
    if(_rnd.Rannyu()<p4){
        // m>1 to avoid the inversion of a single city (can act as a swap if m=2)
        //inversion of the order of m contiguous cities starting from the i-th city
        if(p4 > 0.75){
            //if the probability is high, the inversion is performed on a small number of cities (to escape from local minima)
            way.inversion(_rnd.Rannyu(2,5),_rnd.Rannyu(0,_cities.size()-1));
        }else{
            way.inversion(_rnd.Rannyu(2,_cities.size()-1),_rnd.Rannyu(0,_cities.size()-1));
        }
    }
    return way;
}   
