...
int main(){
    ...
    std::ofstream call{"data/Call_direct.dat"};
    std::ofstream put{"data/Put_direct.dat"};
    ...
    for(int i{0}; i<N; i++){ //data blocking
        ...
        for(int j{0}; j<L; j++){
            Z = rnd.Gauss(0,1);
            S_T = S0*exp((r-0.5*sigma*sigma)*T + sigma*Z*sqrt(T));
            C += exp(-r*T)*std::max(0.,S_T-K);
            P += exp(-r*T)*std::max(0.,K-S_T);
        }
        ave = C/L;
        ave2 = ave*ave;
