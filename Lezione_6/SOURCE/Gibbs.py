...
def main():
   ...
   # Run the simulation for properties with h=0.0
    ...
    print("Running simulation for magnetization with h = 0.02")
   
    h_val = [0.02,0.06,0.1,0.14,0.18,0.22,0.26,0.3,0.34,0.38,0.42,0.46,0.5]
    ...
    for h in h_val:
        ...
        with tqdm(total=num_iterations) as pbar:
            for i in range(num_iterations+1):
                ...
                destination_file = os.path.join(data_dir, f'magnetization{h}.dat')
                extract_last_line(output_file, destination_file, new_temp)
