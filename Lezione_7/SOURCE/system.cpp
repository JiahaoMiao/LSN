...
double System :: Force(int i, int dim){
  ...
  for (int j=0; j<_npart; j++){
    if(i != j){
      ...
      if(dr < _r_cut) f += distance(dim) * (48.0/pow(dr,14) - 24.0/pow(dr,8));
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input >> _n_bins;
        _nprop += _n_bins;
        _bin_size = (_halfside.min() )/(double)_n_bins; // bin size for the gofr, the minimum side is used to avoid empty bins
        _measure_gofr = true;
...
void System :: measure(){ // Measure properties
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(_measure_gofr){
          ...
          if(bin < _n_bins) _measurement(_index_gofr + bin) += 2.0; 
        }
        if(dr < _r_cut){
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure) _measurement(_index_pressure) = _ptail + _rho * _measurement(_index_temp) + 48.0* virial/(3.0*_volume);
